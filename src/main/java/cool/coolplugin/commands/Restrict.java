...
public class Restrict implements CommandExecutor
{
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if (sender.isOp())
        {
            ...
            CoolPluginPlayer.toggleNickRestrict(reciever);

            if (CoolPluginPlayer.hasNickRestrict(reciever))
                sender.sendMessage(colorize("&7>> &a" + reciever.getName() + " has been restricted."));
            else
                sender.sendMessage(colorize("&7>> &a" + reciever.getName() + " is no longer restricted."));
